...
struct retro_core_option_v2_definition option_defs_us[] = {
   ...
   },
   {
      ...
      "video",
      {
         { "off",      "Rec.709 (Match 24-bit)" },
         { "auto",     "Auto (By Region)" },
         { "smptec",   "SMPTE-C (NTSC Studio)" },
         { "ebu",      "EBU (PAL)" },
         { "ntsc1953", "NTSC 1953 (Wide)" },
         { NULL, NULL },
      },
      "off"
   },
   ...
   },
   {
      ...
      "Analog Video Cable",
      NULL,
      "Simulates the cable between the console and the television, including the colour encoding and the television's decoder. 'No Simulation' switches the chain off entirely and hands the framebuffer straight to the display (default). 'RGB (SCART)' is the sharpest output the PSX could produce: the multi-out carries analog RGB, so nothing is modulated and none of the encoding artifacts below can occur - but it is still an analog wire, so it is band-limited by the console's video amplifier and the set's input stage rather than being infinitely sharp. 'S-Video' band-limits colour heavily but keeps luma and colour on separate wires, so fine detail stays clean. 'Composite' sums them onto one wire, which the decoder then has to pull apart again, costing luma detail and adding colour smearing and rainbow fringing on fine patterns. 'RF' models an external modulator - the console itself has none, only a DC jack to power one - narrowing luma further and adding the beat pattern the sound carrier makes against the colour subcarrier, which is strongest on saturated colour. Works at any internal resolution: the extra samples are resolved down before the signal is generated, so higher settings act as supersampling into the cable rather than a wider signal, and the output still carries the sub-pixel detail the decoder produces. Supported by the Vulkan and OpenGL renderers, and ignored by the Software renderer. On OpenGL the filter that removes 'hanging dots' - the fine dotted pattern along horizontal colour edges - needs GL 4.3 or GLES 3.1; on an older driver the simulation still runs, but 'Composite' and 'RF' will show those dots. Applies immediately; no restart needed.",
      NULL,
      "video",
      {
         { "off",       "No Simulation" },
